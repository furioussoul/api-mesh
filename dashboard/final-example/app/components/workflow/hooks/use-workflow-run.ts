...
import { getIncomers, useReactFlow, useStoreApi } from 'reactflow';
      ...
      ssePost(
        ...
        },
        {
            ...
            if (node?.parentId) {
              setWorkflowRunningData(
                produce(workflowRunningData!, (draft) => {
                  ...
                  const iterations = tracing.find(
                    (trace) => trace.node_id === node?.parentId,
                  );
                  const currIteration =
                    iterations?.details![node.data.iteration_index] ||
              ...
              setNodes(newNodes);
              const incomeNodesId = getIncomers(
                { id: data.node_id } as Node,
                newNodes,
                edges,
              )
                .filter(
                  (node) =>
                    node.data._runningStatus === NodeRunningStatus.Succeeded,
                )
                .map((node) => node.id);
              const newEdges = produce(edges, (draft) => {
                draft.forEach((edge) => {
                  if (
                    edge.target === data.node_id &&
                    incomeNodesId.includes(edge.source)
                  )
            ...
            if (nodeParentId) {
              setWorkflowRunningData(
                produce(workflowRunningData!, (draft) => {
                  const tracing = draft.tracing!;
                  const iterations = tracing.find(
                    (trace) => trace.node_id === nodeParentId,
                  ); // the iteration node

                  if (iterations && iterations.details) {
                    const iterationIndex =
                      data.execution_metadata?.iteration_index || 0;
                    ...
                    const nodeIndex = currIteration.findIndex(
                      (node) =>
                        node.node_id === data.node_id &&
                        (node.execution_metadata?.parallel_id ===
                          data.execution_metadata?.parallel_id ||
                          node.parallel_id ===
                            data.execution_metadata?.parallel_id),
                    );
                ...
                produce(workflowRunningData!, (draft) => {
                  const currentIndex = draft.tracing!.findIndex((trace) => {
                    ...
                    return (
                      ...
                      trace.execution_metadata?.parallel_id ===
                        data.execution_metadata?.parallel_id
                    );
                  });
          ...
          onIterationNext: (params) => {
            ...
            setWorkflowRunningData(
              produce(workflowRunningData!, (draft) => {
                const iteration = draft.tracing!.find(
                  (trace) => trace.node_id === data.node_id,
                );
                if (iteration) {
                  if (
                    iteration.details!.length >=
                    iteration.metadata.iterator_length!
                  )
                    return;
                }
            ...
            const newNodes = produce(nodes, (draft) => {
              ...
              currentNode.data._iterationIndex =
          ...
          onIterationFinish: (params) => {
            ...
            setWorkflowRunningData(
              produce(workflowRunningData!, (draft) => {
                const tracing = draft.tracing!;
                const currIterationNode = tracing.find(
                  (trace) => trace.node_id === data.node_id,
                );
